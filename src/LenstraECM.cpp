...
char InnerLoop(std::size_t strt, std::size_t myEnd,
               const mpz_class &k, mpz_class &g,
               const mpz_class &n) {
...
bool LenstraECM(const mpz_class &n, std::size_t maxLoopIter,
    ...
    for (std::size_t i = 0, j = 0; i < maxLoopIter && !factor_found; ++i, j = 0) {
        double p = primes[j++];
        k = 1;
        ...
        while (sectionLength > 0 && nThrdsThisIter > 1) {
            RcppThread::ThreadPool pool(nThrdsThisIter);
            std::vector<mpz_class> vecFactors(nThrdsThisIter);
            std::vector<std::future<char>> myFutures(nThrdsThisIter);
            ...
            myFutures.back() = pool.pushReturn(InnerLoop, strt, myEnd, std::cref(k),
            ...
            const bool bSuccess = std::any_of(vecSuccess.begin(), vecSuccess.end(),
                                              [](char vS) {return vS;});
