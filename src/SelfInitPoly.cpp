...
namespace SIQS {
    ...
    void CreateMyA(const mpz_class &target, const std::vector<int> &facBase,
                   std::vector<int> &AIndexVec, mpz_class &A, int &last) {
        
        if (AIndexVec.empty()) {
            const int smallestLim = 2000;
            const auto smlIt = std::lower_bound(facBase.cbegin(),
                                                facBase.cend(), smallestLim);
            
            const int strt = std::distance(facBase.cbegin(), smlIt);
            
            A = 1;
            int i = strt;
            
            for (const int facSize = facBase.size(); i < facSize && cmp(A, target) < 0; ++i) {
                AIndexVec.push_back(i);
                A *= facBase[i];
            }
            
            // Remove the last two prime factors
            for (int i = 0; i < 2; ++i) {
                A /= facBase[AIndexVec.back()];
                AIndexVec.pop_back();
            }
            ...
            ++last;
            AIndexVec.push_back(last);
        }
        
        mpz_class tarLast = (target / A);
        const auto lstIt = std::lower_bound(facBase.cbegin(),
                                            facBase.cend(), tarLast.get_si());
        A *= (*lstIt);
        AIndexVec.push_back(std::distance(facBase.begin(), lstIt));
    }

    void CreateMyB(const std::vector<mpz_class> &BVec,
                   mpz_class &B, std::size_t BIndex) {
        
        if (BIndex) {
            mpz_class mpzZero = 0;
            B = std::accumulate(BVec.cbegin(), BVec.cend(), mpzZero);
        } else {
            int pow2 = BIndex & ~(BIndex - 1);
            int v = 0;
            
            for (; pow2 >>= 1; ++v)
                ;;
            
            int mySign = std::ceil(static_cast<double>(BIndex) /
                                   static_cast<double>(pow2));
    ...
    std::vector<mpz_class> CreateBVec(const std::vector<int> &AIndexVec,
                                      const std::vector<int> &facBase,
                                      const std::vector<std::size_t> &SieveDist,
                                      const mpz_class &myNum, const mpz_class &A) {
        
        const std::size_t mySize = AIndexVec.size();
        std::vector<mpz_class> BVec(mySize);
        
        mpz_class myQuot, myInverse, primeMpz, test1, test2;
        
        for (std::size_t i = 0; i < mySize; ++i) {
            primeMpz = facBase[AIndexVec[i]];
            myQuot = (A / facBase[AIndexVec[i]]);
            
            mpz_invert(myInverse.get_mpz_t(),
                       myQuot.get_mpz_t(), primeMpz.get_mpz_t());
            
            test1 = myInverse * static_cast<int>(SieveDist[AIndexVec[i] * 2]);
            test1 %= primeMpz;
            
            test2 = myInverse * static_cast<int>(SieveDist[AIndexVec[i] * 2 + 1]);
            test2 %= primeMpz;
            
            if (cmp(test1, test2) < 0) {
                BVec[i] = test1 * myQuot;
            } else {
                BVec[i] = test2 * myQuot;
            }
